 ...
 */
void Camera::update_look_direction(float change_in_yaw_angle, float change_in_pitch_angle) {

    this->yaw_angle += change_in_yaw_angle;
    this->pitch_angle += change_in_pitch_angle;

    this->clamp_camera_pitch_angle();
    //printf("yaw: %f, pitch: %f \n", this->yaw_angle, this->pitch_angle);
 ...
 *
 */
glm::vec3 Camera::yaw_pitch_angles_to_vector(float yaw, float pitch) {
    glm::vec3 vector;
    vector.x = std::cos(pitch) * std::sin(yaw);
    vector.y = std::sin(pitch);
    vector.z = std::cos(pitch) * std::cos(yaw);
    return vector;
}

/**
 * \brief clamps this camera's angle between -pi/2 to pi/2, with some margins
 *
 * The margins are required because if we allow the camera to look straight up or down, opengl's view of the world gets
 * inverted.
 *
 * \author cuppajoeman (2023)
 *
 */
